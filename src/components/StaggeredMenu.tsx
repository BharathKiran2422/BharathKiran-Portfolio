...
type StaggeredMenuProps = {
  position?: 'left' | 'right';
  colors?: string[];
  items?: { label: string; linkComponent?: React.ReactNode; ariaLabel?: string }[];
...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    return () => ctx.revert();
  }, [menuButtonColor, position, changeMenuColorOnOpen, openMenuButtonColor]);
      ...
      <aside id="staggered-menu-panel" ref={panelRef} className="staggered-menu-panel" aria-hidden={!open}>
        ...
        <div className="sm-panel-inner">
          <ul className="sm-panel-list" role="list" data-numbering={displayItemNumbering || undefined}>
            {items && items.length ? (
              items.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={it.label + idx} onClick={closeMenu}>
                   {it.linkComponent ? (
                      it.linkComponent
                    ) : (
                      <a className="sm-panel-item" href={'#'} aria-label={it.ariaLabel} data-index={idx + 1}>
